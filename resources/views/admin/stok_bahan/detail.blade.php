...
<section class="section">
    <div class="section-header">
      <h1>Detail Stok Bahan</h1>
      <div class="section-header-breadcrumb">
        
        @if ($auth->role == 1)
        <div class="breadcrumb-item active"><a href="{{url('/dashboard-admin')}}">Dashboard</a></div>
        <div class="breadcrumb-item active"><a href="{{url('/stok-bahan')}}">Stok Bahan</a></div>
        @else
        <div class="breadcrumb-item active"><a href="{{url('/dashboard-users')}}">Dashboard</a></div>
        <div class="breadcrumb-item active"><a href="{{url('/data-stok-bahan')}}">Stok Bahan</a></div>
        @endif
        
        <div class="breadcrumb-item">Detail Stok Bahan</div>
      </div>
    </div>
    @if (session('Success'))
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-sm-8">
                <h2 class="section-title">Tabel Data Stok Bahan {{$jenis->nama_bahan}}</h2>
...
<form action="{{ url('tambah-stok-bahan') }}" method="POST">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="hidden" name="jenis_bahan_id" value="{{$jenis->id}}">
            </div>
    ...
    <form action="{{ url('edit-stok-tambah-bahan/'. $item->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="modal fade" id="exampleModal2{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Stok Bahan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
    ...
    @foreach ($stok as $item)
    <form action="{{ url('hapus-stok-tambah-bahan/'. $item->id) }}" method="POST">
      @csrf
      @method('DELETE')
      <div class="modal fade" id="exampleModal3{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Hapus Stok Bahan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <p>Apakah anda yakin ingin menghapus data stok bahan
                  @if ($item->warna == "Bawaan")
                  Warna : -
                  @else
                  Warna : {{$item->warna}}
    ...
    <form action="{{ url('edit-stok-ambil-bahan/'. $item->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="modal fade" id="exampleModal4{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Stok Bahan yang Diambil</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
    ...
    @foreach ($stok_ambil as $item)
    <form action="{{ url('hapus-stok-ambil-bahan/'. $item->id) }}" method="POST">
      @csrf
      @method('DELETE')
      <div class="modal fade" id="exampleModal5{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Hapus Stok Bahan yang Diambil</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <p>Apakah anda yakin ingin menghapus data stok bahan yang diambil
                  @if ($item->warna == "Bawaan")
                  Warna : -
                  @else
                  Warna : {{$item->warna}}

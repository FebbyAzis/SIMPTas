...
<section class="section">
    <div class="section-header">
      ...
      <div class="section-header-breadcrumb">
        ...
        @endif
        <div class="breadcrumb-item">Stok Tas</div>
      </div>
    </div>
    @if (session('Success'))
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-sm-9">
                <h2 class="section-title">Tabel Data Stok Tas</h2>
           ...
           <div class="col-sm-3 mt-3 text-right">
            <button type="button" class="btn btn-primary mt-3" data-toggle="modal" data-target="#exampleModal">
                <i class="fas fa-plus"></i>&nbsp; Tambah Jenis Tas
            </button>
        </div>
           @else
                    ...
                    @foreach ($jenis as $no => $j)
                      ...
                      @if($totalWarnaStokAmbil->isNotEmpty())
                        ...
                        @endif
                            ...
                            <td>
                                <center>
                                <a href="{{url('detail-stok-tas/'. $j->id)}}" class="btn btn-success" title="Lihat"><i class="fas fa-eye text-white"></i></a>
                    ...
                    @foreach ($jenis as $no => $j)
                      ...
                      @if($totalWarnaStokAmbil->isNotEmpty())
                        ...
                        @endif
                            ...
                            <td>
                                <center>
                                <a href="{{url('detail-data-stok-tas/'. $j->id)}}" class="btn btn-success" title="Lihat"><i class="fas fa-eye text-white"></i></a>
                                </center>
                            </td>
                        </tr>
                    @endforeach
...
<form action="{{ url('tambah-jenis-tas') }}" method="POST">
    @csrf
    @method('POST')
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="nama_tas" required>
                  </div>
            </div>
...
<form action="{{ url('tambah-stok-tas') }}" method="POST">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="hidden" name="jenis_tas_id" value="{{$item->id}}">
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-danger" data-dismiss="modal">Tutup</button>
              <button type="submit" class="btn btn-primary">Simpan</button>
    ...
    <form action="{{ url('ambil-stok-tas') }}" method="POST">
        ...
        @foreach ($jenis as $item)
        <form action="{{ url('edit-jenis-tas/'. $item->id) }}" method="POST">
          @csrf
          @method('PUT')
          <div class="modal fade" id="exampleModal6{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Edit Jenis Tas</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="modal-body">
                        ...
                        <div class="form-group">
                          <label>Nama Tas</label>
                          <input type="text" class="form-control" name="nama_tas" value="{{$item->nama_tas}}" required>
                        </div>

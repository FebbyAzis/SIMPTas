...
<section class="section">
    <div class="section-header">
      <h1>Detail Stok Tas</h1>
      <div class="section-header-breadcrumb">
        
        @if ($auth->role == 1)
        <div class="breadcrumb-item active"><a href="{{url('/dashboard-admin')}}">Dashboard</a></div>
        <div class="breadcrumb-item active"><a href="{{url('/stok-tas')}}">Stok Tas</a></div>
        @else
        <div class="breadcrumb-item active"><a href="{{url('/dashboard-users')}}">Dashboard</a></div>
        <div class="breadcrumb-item active"><a href="{{url('/data-stok-tas')}}">Stok Tas</a></div>
        @endif
        <div class="breadcrumb-item">Detail Stok Tas</div>
      </div>
    </div>
    @if (session('Success'))
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-sm-8">
                <h2 class="section-title">Tabel Data Stok Tas {{$jenis->nama_tas}}</h2>
                <p class="section-lead">
                  Anda dapat mengelola stok tas pada halaman ini.
                </p>
            </div>
          @if ($auth->role == 1)
...
<form action="{{ url('tambah-stok-tas') }}" method="POST">
    @csrf
    @method('POST')
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel">Tambah Stok Tas</h5>
            ...
            <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="hidden" name="jenis_tas_id" value="{{$jenis->id}}">
            </div>
    ...
    <form action="{{ url('ambil-stok-tas') }}" method="POST">
        @csrf
        @method('POST')
        <div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Ambil Stok Tas</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
    ...
    <form action="{{ url('edit-stok-tambah-tas/'. $item->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="modal fade" id="exampleModal2{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Stok Tas</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
    ...
    @foreach ($stok as $item)
    <form action="{{ url('hapus-stok-tambah-tas/'. $item->id) }}" method="POST">
      @csrf
      @method('DELETE')
      <div class="modal fade" id="exampleModal3{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Hapus Stok Tas</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <p>Apakah anda yakin ingin menghapus data stok tas
                  @if ($item->warna == "Bawaan")
                  Warna : -
                  @else
                  Warna : {{$item->warna}}
    ...
    <form action="{{ url('edit-stok-ambil-tas/'. $item->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="modal fade" id="exampleModal4{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Stok Tas yang Diambil</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
    ...
    @foreach ($stok_ambil as $item)
    <form action="{{ url('hapus-stok-ambil-tas/'. $item->id) }}" method="POST">
      @csrf
      @method('DELETE')
      <div class="modal fade" id="exampleModal5{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Hapus Stok Tas yang Diambil</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <p>Apakah anda yakin ingin menghapus data stok tas yang diambil
                  @if ($item->warna == "Bawaan")
                  Warna : -
                  @else
                  Warna : {{$item->warna}}
